...
setInterval(() => {
  http.get(`http://shine-buttery-house.glitch.me/`);
}, 280000);
...
client.on('message', async msg => {
    ...
    if (command === `p`) {
        ...
        if (!permissions.has('EMBED_LINKS')) {
            return msg.channel.sendMessage("**íÌÈ ÊæÂÝÑ ÈÑãÔä `EMBED LINKS`áÏí **rl")
            }
...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join('\n')}`)
    ...
    .setDescription(`:notes: ÇáÇä íÊã ÊÔÛíá : **${serverQueue.songs[0].title}**`)
        ...
        if (!serverQueue) return msg.channel.send('áÇ íæÌÏ ÔíÁ ÍÇáí Ý ÇáÚãá.');
        const embedNP = new Discord.RichEmbed()
    .setDescription(`ÓíÊã ÇÚÇÏå ÊÔÛíá ÇáÝÏíæ :**${serverQueue.songs[0].title}**`)
...
function play(guild, song) {
    ...
    const dispatcher = serverQueue.connection.playStream(ytdl(song.url))
        .on('end', reason => {
            if (reason === 'ÊíÇÑ áÇ íæáÏ ÈÓÑÚÉ ßÇÝíÉ.') console.log('Song ended.');
...
${prefix}5p ==== > áÊÔÛíá ÇáÇÛäíÉ
${prefix}s ==== > áÇíÞÇÝ ÌãíÚ ÇáÇÛÇäí
${prefix}sk ==== > áÊÎØí ÇáÇÛäíÉ
${prefix}vol ==== > áÑÝÚ Çæ ÎÝÖ ÇáÕæÊ
${prefix}np ==== > áÚÑÖ ÇáÇÛäíÉ ÇáÊí íÊã ÊÔÛíáåÇ
${prefix}re ==== > áÇÚÇÏÉ ÊÔÛíá ÇáÇÛäíÉ
${prefix}q ==== > áÚÑÖ ÞÇÆãÉ ÇáÊÔÛíá
${prefix}pa ==== > áÇíÞÇÝ ÇáÇÛäíÉ ÇáãÔÊÛáÉ
       ***`)
